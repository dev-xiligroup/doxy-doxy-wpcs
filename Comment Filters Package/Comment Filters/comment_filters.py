		...
		if x and equal_pos and searchfuncname == 'apply_filters':
			...
			for param in apply_params:
				...
				if param == param.replace ("$", "\$"):
					...
					the_param = param.replace ("$", "\$")
					only_param = "(" + the_param + "[ ,\[])"
					x1 = re.search( only_param , cur_line)
					xix = re.search("\[\s" + param.replace ("$", "\$") + "\s\]", cur_line )
					if xix:
						indice = 'index'
					else:
						indice = 'string'
					if x1 :
						if x1.start() < x.start():
							linep = indent_line + " * @var <type> " + param.replace ("$", "\$") + " " + dict_apply_filters['result_desc'][indice] + ".\n"
						else:
							fi = fi + 1
							if fi == 1:
								linep = indent_line + " * @param <type> " + param.replace ("$", "\$") + " " + dict_apply_filters['first_param_desc'] + "\n"
							else:
								linep = indent_line + " * @param <type> " + param.replace ("$", "\$") + " " + dict_apply_filters['param_desc'][indice] + "\n"
			...
			for param in apply_params:
				if param == param.replace ("$", "\$"):
					...
					if xix:
						...
						indice = 'string'
					linep = indent_line + " * @param <type> " + param.replace ("$", "\$") + " " + dict_do_action['arg_desc'][indice] +"\n"
				self.view.run_command("insert_snippet", {"contents": linep })
			self.view.run_command("insert_snippet", {"contents": para3 })
		# end do_action
		else:
			...
			if not ( ("$" + searchfuncname) in cur_line ) and  not (( "'" + searchfuncname + "'" ) in cur_line ) and prefix :
				x = re.search(searchfuncname, cur_line)
				apply_params = re.findall ("(\$\w+|\'\w+')", cur_line ) # param and string
				...
				for param in apply_params:
					...
					if param == param.replace ("$", "\$"):
						...
						if x1 :
							if x1.start() < x.start():
								linep = indent_line + " * @var <type> " + param.replace ("$", "\$") + " [" + dict_anonymous['key_or_param'][indice] + "result description].\n"
							else:
								fi = fi + 1
								if fi == 1:
									linep = indent_line + " * @param <type> " + param.replace ("$", "\$") + " " + dict_anonymous['first_param_desc'] + "\n"
								else:
									linep = indent_line + " * @param <type> " + param.replace ("$", "\$") + " " + dict_anonymous['param_desc'][indice] + "\n"
					self.view.run_command("insert_snippet", {"contents": linep })
